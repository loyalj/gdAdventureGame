...
signal start_scene_transition(transitionName)
signal scene_switched(newScene)
signal finish_scene_transition()
...
func set_root_node(node):
	rootNode = node
	currentScene = rootNode.get_child(rootNode.get_child_count() - 1)
...
func _deferred_switch_scene(newScenePath):
	...
	rootNode.add_child(currentScene)
	rootNode.move_child(currentScene, rootNode.get_child_count() - 1)
	scene_switched.emit(currentScene)
...
func _deferred_transition_scene(newScenePath, transitionName):
	...
	start_scene_transition.emit(transitionName)
